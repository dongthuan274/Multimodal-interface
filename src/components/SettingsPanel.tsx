...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ settings, onSettingsChange }) => {
  ...
  const handleFilterUpdate = (key: keyof SearchSettings['filters'], value: boolean) =>
    onSettingsChange({ ...settings, filters: { ...settings.filters, [key]: value } });

  return (
    <aside className="w-full lg:w-72 flex-shrink-0 bg-slate-800 p-5 border-r border-slate-700/50 overflow-y-auto">
      <h2 className="text-lg font-bold mb-5 text-slate-100">Search Settings</h2>
      ...
      <Section title="Search Method">
        ...
        >
          {SEARCH_METHODS.map(m => <option key={m} value={m}>{m}</option>)}
        </select>
      </Section>
      ...
      <Section title="Layout">
        <div>
          ...
          <select
            ...
            className="w-full p-2 bg-slate-700 border border-slate-600 rounded-md focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-colors"
          >
            {RESULTS_PER_ROW_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
          </select>
        </div>
      </Section>

      <Section title="Processing">
        <Toggle label="OCR" checked={settings.ocr} onChange={v => handleUpdate('ocr', v)} />
      </Section>

      <Section title="Additional Filters">
        <Toggle label="Date" checked={settings.filters.date} onChange={v => handleFilterUpdate('date', v)} />
        <Toggle label="Relevance" checked={settings.filters.relevance} onChange={v => handleFilterUpdate('relevance', v)} />
        <Toggle label="Similarity" checked={settings.filters.similarity} onChange={v => handleFilterUpdate('similarity', v)} />
      </Section>
    </aside>
  );
};
